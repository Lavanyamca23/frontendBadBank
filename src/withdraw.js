...
function Withdraw(){
    ...
    useEffect(()=>{
        async function fetchData(){
            if(name&&pass){
                await axios.get('https://server-tqam.onrender.com/data').then(res=>{console.log(res.data);
                setData(res.data);
    ...
    async function handleClick(e){
        ...
        for(let i=0; i<data.length; i++){
            if(data[i].name === name && data[i].password === pass){
                await axios.put(`https://server-tqam.onrender.com/update/${data[i]._id}`,{amount:data[i].amount - Number(withdraw)});
